...
decodeOther(Gate *circuit, int gateIndex)
{
    ...
    if (1==current->type){   //intermediate gate
        ...
        if(('X'==current->fName[0])||('X'==current->fName[0])){
            int i;
            wire->key=malloc(KEYLEN);
            for(i=0;i<KEYLEN;i++) wire->key[i]=current->table[ROW][COL][i]^K0[i]^K1[i];
            wire->gb=current->table[ROW][COL][2*KEYLEN-1];
            return wire;
        }
        else{ 
...
decode(Gate *circuit, int gateIndex)
{
    ...
    if(('X'==current->fName[0])||('X'==current->fName[1])){
        return current->table[ROW][COL][KEYLEN-1]^K0[KEYLEN-1]^K1[KEYLEN-1];
    }
    else{
