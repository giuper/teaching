...
void encodeXOROutput(Gate *,Gate *);
void encodeInput(Gate *,Gate *,int *);
void encodeIntermediate(Gate *,Gate *);
void encodeXORIntermediate(Gate *,Gate *);

/* circuit is the circuit
   numOfGates is the number of gates in the circuit
   x is the 0/1 assignment to the x0,x1,...
*/
...
printf("\n\n");
    ...
    for(i=0;i<numOfGates;i++){
        ...
        switch (current->type){
            ...
            case 1:
                if (('X'==current->fName[0])||('X'==current->fName[1]))
                    encodeXORIntermediate(circuit,current);
                else
            ...
            case 2:
                if (('X'==current->fName[0])||('X'==current->fName[1]))
                    encodeXOROutput(circuit,current);
                else
...
void
encodeXOROutput(Gate *circuit, Gate *current)
{
    ...
    for(r=0;r<2;r++){
        for(c=0;c<2;c++){
            res=current->funct(r^b0,c^b1);     
            K0=p0->keys[r^b0]; K1=p1->keys[c^b1];
            RAND_bytes((unsigned char *)plaintext,KEYLEN);
            plaintext[KEYLEN-1]=(unsigned char)res;
            current->table[r][c]=(unsigned char *)malloc(KEYLEN);
            for(i=0;i<KEYLEN;i++) current->table[r][c][i]=plaintext[i]^K0[i]^K1[i];
...
void
encodeXORIntermediate(Gate *circuit,Gate *current)
{
    ...
    for(r=0;r<2;r++){
        for(c=0;c<2;c++){
            ...
            current->table[r][c]=(unsigned char *)malloc(2*KEYLEN);
            for(i=0;i<KEYLEN;i++)
                current->table[r][c][i]=current->keys[res][i]^K0[i]^K1[i];
            current->table[r][c][2*KEYLEN-1]=(unsigned char)res^ngb;
#ifdef DEBUG
            printf("Intermediate XOR gate %d\ttable[%d][%d] holding result for (%d,%d)\n",current->index,r,c,b0^r,b1^c);
            printf("Plaintext: \n");
            BIO_dump_fp (stdout,(const char *)current->keys[res],KEYLEN);
            printf("Bit: %d\n",res^ngb);
            printf("with key:\n");
            BIO_dump_fp (stdout,(const char *)K0,KEYLEN);
            printf("and key:\n");
